...
*
      ...
      character*300 string,filename,filename1,filename2
      ...
      character*14  full,fullspec(imax),cdum
...
*
      ...
      print*,' this option can be selected through Unit selection'
      print*,' in the VALD3 interface.'

      IARG=0
      filename=' '
      CALL GETARG(IARG+1,filename)
      if(filename.eq.' ') then
        print*
        print *, 'VALD3 ''extract all/element/stellar'' '
     *         //'(long format) filename?'
        read(*,'(A)') filename
      endif
      open(10,file=filename,form='formatted',status='old')
      filename1=filename

      CALL GETARG(IARG+2,filename2)
  10  if(filename2.eq.' ') then
        jj=-1
        do j=1,index(filename,' ')
          if(filename(j:j).eq.'.') jj=j
        enddo
        if(jj.gt.0) then
          filename2=filename(1:jj)//'turbo'
        else
          filename2=' '
          jj=1
        endif
        write(*,'(''Output bsyn/turbospectrum line filename? ('',A,'
     *        //''')'')') filename2(1:jj+5)
        read(*,'(A)') filename
        if(filename.eq.' ') filename=filename2
      else
        filename=filename2
      endif
      if(filename1.eq.filename) then
        print*,'Do you want to overwrtite the input file?'
     *       //' Bad idea! Try again.'
        go to 10
      endif
...
* for extract all:
*'_          Kurucz CoII 2006   1 wl:K06   1 gf:K06   1 K06   1 K06   1 K06   1 K06   1 K06   1 K06   1 K06   hfs:DP,DP Co+'
*'_          Kurucz ScI 2009    2 wl:K09   2 gf:K09   2 K09   2 K09   2 K09   2 K09   2 K09   2 K09   2 K09   hfs:BBAOKac,BBAOKa Sc'
*'_      C   Kurucz MnI 2007    8 wl:K07   8 gf:K07   8 K07   8 K07   8 K07   8 K07   8 K07   8 K07   8 K07    Mn'
*'_                             5 wl:PPN2012   5 gf:PPN2012   5 PPN2012   5 PPN2012   5 PPN2012   5 PPN2012   5 PPN2012   5 PPN2012   5 PPN2012    (50)TiO'
*'_                             4 wl:PPN2012   4 gf:PPN2012   4 PPN2012   4 PPN2012   4 PPN2012   4 PPN2012   4 PPN2012   4 PPN2012   4 PPN2012    (49)TiO'
*'_                             7 wl:PPN2012   7 gf:PPN2012   7 PPN2012   7 PPN2012   7 PPN2012   7 PPN2012   7 PPN2012   7 PPN2012   7 PPN2012    (48)TiO'
*'_                             3 wl:PPN2012   3 gf:PPN2012   3 PPN2012   3 PPN2012   3 PPN2012   3 PPN2012   3 PPN2012   3 PPN2012   3 PPN2012    (47)TiO'
*'_                             9 wl:BRW   9 gf:BRW   9 BRW   9 BRW   9 BRW   9 BRW   9 BRW   9 BRW   9 BRW    (12)C(14)N'
*
* and for extract stellar: :-(
*'E 0.06     Sc-Ni Wisconsin   13 wl:LBT,iso:K08  13 gf:WLSCow  13 LBT  13 LBT  14 K08  14 K08  14 K08  14 K08  13 WLSCow    (64)Ni'
*'E 0.06     Sc-Ni Wisconsin   13 wl:LBT,iso:K08  13 gf:WLSCow  13 LBT  13 LBT  14 K08  14 K08  14 K08  14 K08  13 WLSCow    (61)Ni'
*'E 0.06     Sc-Ni Wisconsin   13 wl:LBT,iso:K08  13 gf:WLSCow  13 LBT  13 LBT  14 K08  14 K08  14 K08  14 K08  13 WLSCow    (60)Ni'
*'E 0.06     Sc-Ni Wisconsin   13 wl:LBT,iso:K08  13 gf:WLSCow  13 LBT  13 LBT  14 K08  14 K08  14 K08  14 K08  13 WLSCow    (58)Ni'
*'_          Cu 1 - Ba 2 Bell   1 wl:MW   1 gf:MW   1 MW   1 MW   1 MW   1 MW   1 MW   1 MW   1 MW    Ba'

* read species identification from string4 and find out species
*
            do j=len(string4),1,-1
              if(string4(j:j).ne.' ') then
                jjj=j
                go to 11
              endif
            enddo
  ...
  11        do j=jjj,1,-1
              if(string4(j:j).eq.' ') then
                jj=j
                go to 12
              endif
            enddo

  12        cdum=string4(jj+1:jjj)
            j=index(cdum,' ')
            if (j.le.1) then
...
c            write(*,'(I,A)') ielem,cdum(1:index(cdum,' '))
              j=0
              j=index(string4,'(')
